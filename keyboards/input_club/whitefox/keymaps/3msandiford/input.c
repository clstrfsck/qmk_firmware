...
static zint_t dictionary_size = 0;
...
int tolower( int ch ) {
   if ( ch >= 'A' && ch <= 'Z' ) {
      return ch | ( 'a' - 'A' ); // Assume some kind of asciish coding
   }
   return ch;
}
 ...
 */
bool z_sread_aread( int argc, zword_t * argv ) {
   int in_size, out_size, terminator;
   char *cbuf, *buffer;
   ...
   /* Read the line */
   terminator = get_line( cbuf, argv[3] );
   if (!terminator) {
      return false;
   }
 ...
 */
int get_line( char *cbuf, zword_t action_routine ) {
   char *buffer;
   ...
   do {
      c = input_line( buflen, buffer, &read_size );
      status = 0;
...
static void tokenise_line( zword_t char_buf, zword_t token_buf, zword_t dictionary, zword_t flag ) {
   ...
   dictionary_size = ( zint_t ) get_word( dictionary );
   dictionary_offset = dictionary + 2;

   /* Calculate the binary chop start position */
   if ( dictionary_size > 0 ) {
      long word_index = dictionary_size / 2;
      chop = 1;
      do
         chop *= 2;
      while ( word_index /= 2 );
...
static zword_t find_word( int len, const char *cp, long chop ) {
   ...
   if ( dictionary_size > 0 ) {
      ...
      while ( chop ) {
         ...
         if ( ( status = word[0] - ( zint_t ) get_word( offset + 0 ) ) == 0 &&
              ( status = word[1] - ( zint_t ) get_word( offset + 2 ) ) == 0 )
      ...
      for ( word_index = 0; word_index < -dictionary_size; word_index++ ) {
         ...
         /* If word matches then return dictionary offset */
         if ( ( word[0] - ( zint_t ) get_word( offset + 0 ) ) == 0 &&
              ( word[1] - ( zint_t ) get_word( offset + 2 ) ) == 0 )
            return ( ( zword_t ) offset );
      }
   }
