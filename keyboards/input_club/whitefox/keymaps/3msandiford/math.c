...
{
   store_operand( (zword_t)(( zint_t ) a * ( zint_t ) b) );
}
...
void z_log_shift( zword_t a, zword_t b )
{
   ...
   if ( ( zint_t ) b > 0 )
      store_operand( (zword_t)( a << ( zint_t ) b ) );
   else
      store_operand( (zword_t)( a >> -( ( zint_t ) b ) ) );
...
void z_random( zword_t a ) {
   if ( a == 0 ) {
      ...
      set_random_seed( ( unsigned int ) ( ( a ^ 0xFFFF ) + 1 ) );
      store_operand( 0 );
   }
   else                         /* (a > 0) */
      store_operand( (zword_t)( get_random( a ) + 1) );
}
...
{
   conditional_jump( ( zint_t ) a < ( zint_t ) b );
}
