...
static const char * const lower_a_to_z = "abcdefghijklmnopqrstuvwxyz";
static const char * const upper_a_to_z = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
static const char * const v3_lookup_table[3] = {
   lower_a_to_z,
   upper_a_to_z,
   " \n0123456789.,!?_#'\"/\\-:()"
};
...
int z_call( int argc, zword_t * argv, int type ) {
   ...
   int args = ( unsigned int ) read_code_byte(  );
   while ( --args >= 0 ) {
...
void z_jump( zword_t offset ) {
   pc = ( unsigned long ) ( pc + ( zint_t ) offset - 2 );
}                               /* z_jump */
